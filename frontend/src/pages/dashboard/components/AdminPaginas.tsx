...
export default function ListaPaginas({
  ...
  return (
    <>
      {paginas.data?.managingPages?.length ? (
        <ListGroup>
          {paginas.data.managingPages.map((item: any) => (
            <MemoizedPagina
              key={item.id}
              pagina={item}
        ...
        <Alert variant="warning">
          <Alert.Heading>Este usuario no ha registrado páginas para monitorear</Alert.Heading>
          <p>
            Esperemos hasta que registre una nueva página.
          </p>
        </Alert>
      )}
